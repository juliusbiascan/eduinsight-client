...
import { Config, WindowIdentifier } from '@/shared/constants';
...
export const DeviceSetup: React.FC = () => {
  ...
  const checkConnections = async () => {
    ...
    const socketConnected = await api.socket.test(Config.SOCKET_URL);
    ...
    const dbConnected = await api.database.connect(Config.DATABASE_URL);
  ...
  const renderRetryButton = () => {
    if (status !== SetupStatus.ConnectionFailed) return null;
    ...
    return (
      ...
      >
        <FiRefreshCw className={`w-3 h-3 mr-1 ${isRetrying ? 'animate-spin' : ''}`} />
        {isRetrying ? `Retrying (Attempt ${retryCount + 1})...` : 'Retry Connection'}
      </Button>
    );
  };
    ...
    >
      <div className="h-full grid grid-cols-[300px_1fr] gap-3 bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-3"> {/* Adjusted grid and spacing */}
        ...
        <div className="bg-gray-50/80 rounded-xl p-3 flex flex-col gap-3"> {/* Adjusted padding and gap */}
          ...
          {status === SetupStatus.ConnectionFailed && (
