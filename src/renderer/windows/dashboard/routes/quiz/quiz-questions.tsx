...
const QuizQuestions: React.FC = () => {
  ...
  const [questionType, setQuestionType] = useState<QuestionType>(
    state.questionType || questionTypes[0],
  );
  ...
  const [answerType, setAnswerType] = useState(
    editingQuestion
      ? JSON.parse(editingQuestion.options).filter(
          (opt: AnswerOption) => opt.isCorrect,
        ).length > 1
        ? 'multiple'
        : 'single'
      : 'single',
  ...
  const [answerOptions, setAnswerOptions] = useState<AnswerOption[]>(
    editingQuestion
      ? JSON.parse(editingQuestion.options).map(
          (opt: AnswerOption, index: number) => ({ ...opt, id: index + 1 }),
        )
      : [
  ...
  useEffect(() => {
    if (editingQuestion) {
      setQuestionType(
        questionTypes.find((type) => type.label === editingQuestion.type) ||
          questionTypes[0],
      );

      // Set answer type based on the number of correct answers
      const correctAnswersCount = JSON.parse(editingQuestion.options).filter(
        (opt: AnswerOption) => opt.isCorrect,
      ).length;
      ...
      const setInitialOrder = async () => {
        ...
        if (quiz?.[0]?.questions) {
          const maxOrder = Math.max(
            ...quiz[0].questions.map((q) => q.order ?? 0),
          );
  ...
  const handleSaveQuestion = async () => {
    ...
    if (questionType.label === 'Identification') {
      ...
      try {
        let response;
        if (editingQuestion) {
          response = await api.database.updateQuizQuestion(
            editingQuestion.id,
            questionData,
          );
        } else {
          response = await api.database.createQuizQuestionByQuizId(
            quizId,
            questionData,
          );
        }
        ...
        if (response) {
          message.success(
            `Question ${editingQuestion ? 'updated' : 'saved'} successfully`,
          );
    ...
    if (questionType.label === 'Fill in the Blank') {
      ...
      if (!question.includes('_')) {
        message.error(
          'Please include at least one blank (using underscores) in your question',
        );
        return;
      }
      ...
      if (blankCount !== answerOptions.length) {
        message.error(
          `Number of blanks (${blankCount}) doesn't match number of answers (${answerOptions.length})`,
        );
        return;
      }
    ...
    if (
      answerOptions.filter((option) => option.text.trim()).length < 2 &&
    ...
    if (
      !answerOptions.some((option) => option.isCorrect) &&
      questionType.label !== 'Fill in the Blank'
    ) {
    ...
    if (questionType.label === 'Identification') {
      ...
      setAnswerOptions(
        answerOptions.map((opt) => ({ ...opt, isCorrect: true })),
      );
    }
                    ...
                    <Card key={option.id} style={{ marginBottom: '8px' }}>
                      <Space>
                        ...
                        <Input
                          value={option.text}
                          onChange={(e) =>
                            handleAnswerOptionChange(option.id, e.target.value)
                          }
        ...
        return (
          <Space direction="vertical" style={{ width: '100%' }}>
            <Typography.Title level={5}>
              Enter your question text and use underscores (_____) to mark
              blanks
            </Typography.Title>
            <TextArea
              rows={4}
        ...
        return (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Typography.Text type="secondary" style={{ marginBottom: '16px' }}>
              Add multiple possible answers that will be considered correct. The
              student's answer will be marked as correct if it matches any of
      ...
      case 'Enumeration':
        return (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Typography.Text type="secondary" style={{ marginBottom: '16px' }}>
              Add all possible correct answers. Students must list the exact
              number of required items. The order of answers doesn't matter.
            </Typography.Text>
            {answerOptions.map((option) => (
              <Card key={option.id} style={{ marginBottom: '8px' }}>
                <Space>
      ...
      >
        <Space>
          ...
          <Select
            ...
            style={{ width: 180 }}
            onChange={(value) => {
              const newType = questionTypes.find(
        ...
        <Space>
          <Select
            ...
            style={{ width: 120 }}
            onChange={(value) => setPoints(parseInt(value))}
          >
            {[1, 2, 3].map((value) => (
