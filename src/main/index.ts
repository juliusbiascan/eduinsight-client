...
import { createSocketConnection, isSocketConnected, getConnectionStatus, disconnectSocket, removeAllListeners, testHttpConnection } from './lib/socket-manager';
...
function setupSocketEventListeners(socket: Socket) {
  ...
  socket
    ...
    .on('mouse_move', ({ clientX, clientY, clientWidth, clientHeight }) => {
      ...
      robot.moveMouse(clientX * width / clientWidth, clientY * height / clientHeight);
    })
    .on('mouse_click', (data) => {
    ...
    .on('mouse_drag', ({ direction, clientX, clientY, clientWidth, clientHeight }) => {
      ...
      const { width, height } = screen.getPrimaryDisplay().workAreaSize;
      robot.dragMouse(clientX * width / clientWidth, clientY * height / clientHeight);
    })
    .on('keyboard', (keys) => {
      try {
...
function handleOnReady() {
  ...
  if (connectionUrl) {
    testHttpConnection(connectionUrl)
      .then((isReachable) => isReachable ? createSocketConnection(connectionUrl) : Promise.reject("Server is not reachable"))
      .then((socket) => {
